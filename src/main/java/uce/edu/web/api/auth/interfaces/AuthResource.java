...
public class AuthResource {
    ...
    public TokenResponse token(
        ...
        if (semilla != null && !semilla.isEmpty() && tiempo != null) {
            String role = "user";
            if ("admin".equalsIgnoreCase(semilla)) {
                role = "admin";
            }
        ...
        if (user != null && !user.isEmpty() && password != null && !password.isEmpty()) {
            uce.edu.web.api.auth.domain.Usuario usuario = usuarioService.validarCredenciales(user, password);
        ...
        throw new jakarta.ws.rs.WebApplicationException(
                "Debe proporcionar (user y password) O (semilla y tiempo)",
